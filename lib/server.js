...
Server.prototype.receive = function (ciphertext, ip, port) {
    ...
    decipher.on('end', async function () {
        try {
            ...
            await fs_appendFile(`${this.filename}-${ip}-${port}.txt`,
                JSON.stringify(Object.assign({timestamp: tokens[3]}, tokens[1])) + '\n');
        } catch (ex) {
            console.error(ex);
        }
    });
