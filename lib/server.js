...
/**
 * create a server instance, each server can handle multiple connections.
 * @param opts
 * @return {Server}
 * @constructor
 */
 ...
 * @return {Promise<void>}
 */
Server.prototype.logFile = async function (data, ip, filename) {
    try {
        ...
        await fs_appendFile(`${filename}-${ip}-${wid}.txt`,
            JSON.stringify(log) + '\n');
    } catch (ex) {
        console.error(ex);
    }
...
/**
 * receive incoming message which is encrypted or not.
 * @param ciphertext
 * @param ip
 * @param port
 * @return {Promise<void>}
...
Server.prototype.receive = async function (ciphertext, ip, port) {
    if (this.password == null) {
        this.logFile(ciphertext, ip, this.filename);
        return;
    }
    ...
    decipher.on('end', async function () {
        that.logFile(plaintext, ip, that.filename);
    });
